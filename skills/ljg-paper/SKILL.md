...
输出结构依据 `references/template.md`。禁止参考 `~/doc/notes/` 中已有论文文件的章节结构——旧文件可能使用过期模板。
...
如果论文有一张承载全文核心思路的总览图（overview / architecture diagram，通常是 Figure 1），提取并保存到 `~/doc/notes/images/`，文件名 `{identifier}--paper-{简短标题}-overview.png`。
...
按 Denote 规范获取时间戳，读 `references/template.md`，写入 `~/doc/notes/`。
